...
def excluir_usuario(root, cpf_adm_executor, cpf_usuario_alvo):
    ...
    if messagebox.askyesno("Confirmar Exclusão", f"Excluir o usuário CPF {cpf_usuario_alvo}?", icon='warning'):
        ...
        try:
            ...
            acao = f"Usuário excluído por ADM (CPF: {cpf_adm_executor})"
            bd.inserir_log(cpf_adm=cpf_adm_executor, cpf_alvo=cpf_usuario_alvo, acao=acao)

            bd.arquivar_usuario(cpf_usuario_alvo)
