...
class BancoDeDados:
    ...
    def inserir_log(self, cpf_adm, cpf_alvo, acao):
        ...
        try:
            ...
            if cpf_alvo is not None and "excluído" not in acao and "excluiu" not in acao:
                ...
                if cursor.fetchone() is None:
                    print(
                        f"[BD ERROR] Falha ao inserir log: CPF do usuário alvo ({cpf_alvo}) não encontrado na tabela Pessoa.")
                    raise sqlite3.IntegrityError(
                        f"FOREIGN KEY constraint failed: Pessoa CPF {cpf_alvo} não existe.")
    ...
    def criar_tabela_arquivada(self):
        """Cria a tabela Pessoa_Arquivada, se não existir."""
        try:
            cursor = self.conn.cursor()
            # Criando a tabela de arquivamento com a mesma estrutura de Pessoa, mais o campo 'data_arquivamento'
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS Pessoa_Arquivada (
                    cpf TEXT PRIMARY KEY,
                    nome TEXT NOT NULL,
                    login TEXT NOT NULL UNIQUE,
                    senha TEXT NOT NULL,
                    ...
                    email TEXT NOT NULL UNIQUE,
                    data_cadastro TEXT,
                    data_arquivamento TEXT -- Campo para registrar quando o usuário foi arquivado
                )
            """)
            self.conn.commit()
            print("Tabela Pessoa_Arquivada criada com sucesso!")
        except sqlite3.Error as e:
            print(f"[BD ERROR] Erro ao criar a tabela Pessoa_Arquivada: {e}")
            raise

    def adicionar_coluna_data_arquivamento(self):
        """Adiciona a coluna data_arquivamento à tabela Pessoa_Arquivada."""
        try:
            cursor = self.conn.cursor()
            cursor.execute("ALTER TABLE Pessoa_Arquivada ADD COLUMN data_arquivamento TEXT;")
            self.conn.commit()
            print("Coluna data_arquivamento adicionada com sucesso!")
        except sqlite3.Error as e:
            print(f"[BD ERROR] Erro ao adicionar coluna data_arquivamento: {e}")
            raise

    def arquivar_usuario(self, cpf):
        """Move o usuário para a tabela de Arquivo antes de excluir."""
        try:
            cursor = self.conn.cursor()
            data_arquivamento = datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # Data e hora do arquivamento

            # Insere os dados do usuário na tabela de Arquivamento (Pessoa_Arquivada)
            cursor.execute("""
                INSERT INTO Pessoa_Arquivada (
                    cpf, nome, login, senha, bloco, numero_ap, email, data_cadastro, data_arquivamento
                )
                SELECT cpf, nome, login, senha, bloco, numero_ap, email, data_cadastro, ?
                FROM Pessoa
                WHERE cpf = ?
            """, (data_arquivamento, cpf))

            # Depois de arquivar, deleta o usuário da tabela Pessoa
            cursor.execute("DELETE FROM Pessoa WHERE cpf = ?", (cpf,))
            self.conn.commit()
            print(f"[BD DEBUG] Usuário {cpf} movido para Pessoa_Arquivada e removido de Pessoa.")
        except sqlite3.Error as e:
            print(f"[BD ERROR] Erro ao arquivar e deletar usuário {cpf}: {e}")
            raise



    def get_nome_adm(self, cpf_adm):
        """Obtém o nome do administrador com base no CPF."""
        try:
            cursor = self.conn.cursor()
            cursor.execute("SELECT nome FROM Adm WHERE cpf = ?", (cpf_adm,))
            nome = cursor.fetchone()

            if nome:
                return nome[0]  # Retorna o nome
            else:
                print(f"[BD ERROR] Admin com CPF {cpf_adm} não encontrado.")
                return None
        except sqlite3.Error as e:
            print(f"[BD ERROR] Erro ao buscar nome do ADM: {e}")
            raise

    def get_nome_alvo(self, cpf_alvo):
        """Obtém o nome do usuário alvo com base no CPF."""
        try:
            cursor = self.conn.cursor()
            cursor.execute("SELECT nome FROM Pessoa WHERE cpf = ?", (cpf_alvo,))
            nome = cursor.fetchone()

            if nome:
                return nome[0]  # Retorna o nome
            else:
                print(f"[BD ERROR] Usuário com CPF {cpf_alvo} não encontrado.")
                return None
        except sqlite3.Error as e:
            print(f"[BD ERROR] Erro ao buscar nome do alvo: {e}")
            raise

    def excluir_usuario_com_arquivamento(self, cpf_adm_executor, cpf_usuario_alvo):
        """Exclui um usuário pelo ADM, mas antes arquiva seus dados."""
        print(f"[ADM DEBUG] Iniciando exclusão do Usuário CPF {cpf_usuario_alvo} pelo ADM CPF {cpf_adm_executor}")

        # Garante que a tabela Pessoa_Arquivada exista antes de tentar arquivar o usuário
        self.criar_tabela_arquivada()

        # Obtemos os nomes do ADM e do usuário alvo
        nome_adm = self.get_nome_adm(cpf_adm_executor)
        nome_alvo = self.get_nome_alvo(cpf_usuario_alvo)

        # Arquiva o usuário
        self.arquivar_usuario(cpf_usuario_alvo)

        # Registra o log de exclusão (mesmo que o usuário esteja sendo movido para outra tabela)
        acao = f"Usuário excluído por ADM ({nome_adm})"
        log_id = self.inserir_log(cpf_adm_executor, cpf_usuario_alvo, acao)  # Registra a ação no log

        # Processo de exclusão do usuário (que já foi arquivado)
        try:
            cursor = self.conn.cursor()
            cursor.execute("DELETE FROM Pessoa WHERE cpf = ?", (cpf_usuario_alvo,))
            self.conn.commit()
            print(f"[ADM DEBUG] Usuário CPF {cpf_usuario_alvo} excluído.")
        except sqlite3.Error as e:
            print(f"[ADM ERROR] Erro ao excluir usuário: {e}")
            raise
