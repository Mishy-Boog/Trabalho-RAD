...
class BancoDeDados:
    ...
    def inserir_log(self, cpf_adm, cpf_alvo, acao):
        ...
        try:
            ...
            # Para a exclusão, pegamos o nome e CPF do usuário antes de deletá-lo
            if "excluído" in acao or "excluiu" in acao:  # Verifica se a ação é exclusão
                if cpf_alvo is not None:
                    ...
                    nome_alvo, cpf_alvo = usuario_alvo  # Aqui pegamos nome e cpf do usuário alvo

                    # Agora, modificamos a ação para incluir o nome e CPF do usuário excluído
                    acao = f"Usuário excluído - Nome: {nome_alvo} | CPF: {cpf_alvo}"
