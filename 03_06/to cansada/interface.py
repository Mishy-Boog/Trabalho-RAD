...
root.geometry("300x400")  # Cor de fundo da janela
...
def editar_dados_usuario(cpf):
    ...
    editar_window.configure(bg='lightblue')
...
def excluir_conta(cpf):
    ...
    if resposta:
        ...
        if nome:
            ...
            bd.conn.commit()
            # Agora passando o responsável como o usuário logado
            bd.inserir_log(nome, "Usuário excluído", bd.usuario_logado if bd.usuario_logado else "Desconhecido")
...
def fazer_login():
    ...
    if bd.conn:
        ...
        if bd.validar_login(login, senha):  # Aqui estamos chamando o método corretamente na instância bd
...
tk.Label(root, text="Login", bg="pink", font=("Arial", 14, "bold"), fg="black", ).pack(pady=(20, 5))
entry_login = tk.Entry(root, bd=2, highlightthickness=2, highlightbackground="pink", highlightcolor="gray")
entry_login.pack()

tk.Label(root, text="Senha", bg="pink", font=("Arial", 14, "bold"), fg="black").pack(pady=(20, 5))
entry_senha = tk.Entry(root, show=">", bd=2, highlightthickness=2, highlightbackground="pink", highlightcolor="gray")
entry_senha.pack()

tk.Button(root, text="Entrar", command=fazer_login, relief="raised", font=("Arial", 8, "bold"),
          bg="light blue", fg="black", padx=7, pady=3, cursor="hand2", ).pack(pady=30)
tk.Button(root, text="Cadastrar-se", command=abrir_cadastro, relief="raised", font=("Arial", 8, "bold"),
          bg="light blue", fg="black", padx=8, pady=3, cursor="boat", ).pack(pady=20)
